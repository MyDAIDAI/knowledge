...
上面的代码中，`prepareFreshStack`函数将当前`FiberRootNode`类型的节点传入，使用其`root.current`属性创建`workInProgress`，`root.current`为当前容器的`FiberNode`节点，也就是`FiberNode(HostRoot)`，`workLoopSync`以及`performUnitOfWork`函数实现了对`FiberTree`的创建，其中`beginWork`是创建`FiberNode`，`completeUnitOfWork`为创建`stateNode`并构建当前节点的`DOM`树的过程。
...
![alt text](react-begin-loop1.png)
