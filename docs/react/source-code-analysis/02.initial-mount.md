...
上面的同步模式与并发模式的主要区别为渲染根节点的过程，**同步模式**创建`FiberTree`的过程**不可中断**，**并发模式**则可以被**高优先级任务中断**，而`commitRoot`过程则是一致的，都是同步执行。
...
📢 注意：由于`commitRoot`是**一次执行完成的挂载过程**，为了避免浏览器产生闪烁以及重绘等，`React`内部进行了优化，也就是在`render`的`complete`阶段，就将子树的整个`DOM`树构建完成了，并将其对应的`flags`冒泡到父节点的`subtreeFlags`属性中，后续在`commit`阶段直接比较该属性进行相应的子树挂载即可。下面会进行详细的解析：
...
- 2.开启工作循环：`workLoopSync`以及`performUnitOfWork`函数实现了对`FiberTree`的创建，其中`beginWork`是创建`FiberNode`，`completeUnitOfWork`为创建`stateNode`并构建以当前节点为根结点的`DOM`树的过程。
...
这是第一次`workLoop`循环，当前的`workInProgress === FiberNode(HostRoot)`，进入`beginWork`函数，根据当前`tag === 3`，进入`updateHostRoot`函数，然后进入`reconcileChildren`函数中，此时，会将 `root.render(<App />)`代码中的`<App />`函数组件，经过`babel`编译后的`ReactElement`对象作为`nextChildren`传入`reconcileChildFibers`作为函数，该函数为执行`ChildReconciler(true)`后返回的函数，也就是`ChildReconciler`函数中的闭包`reconcileChildFibers`函数，其中`shouldTrackSideEffects`参数为`true`。后面继续进入`reconcileSingleElement`函数，该函数的主要作用是创建`FiberNode(<App />)`，并将其`return`属性，指向当前`FiberNode(HostRoot)`。后续将当前新创建的`FiberNode(<App />)`作为参数传入`placeSingleChild`函数，添加`flags`，其`shouldTrackSideEffects === true`，则`FiberNode(<App />).flags === Placement`，执行到当前的内存数据接口如下：
...
![alt text](./images/react-work-loop5.png)，大致流程与上次循环一致，不同点为没有其子元素，函数返回的为`null`，其没有子节点，于是进入`performUnitOfWork`函数中的`completeUnitOfWork`函数。当前内存中数据结构如下：
![alt text](./images/image-30.png)

上面整个`beginWork`阶段如下图所示：
![alt text](./images/beginWork.png)

#### completeUnitOfWork阶段

上面的`beginWork`执行到`FiberNode(div-A)`节点后，返回的`workInProgress.child === null`，表明`FiberNode(div-A)`下面没有子节点，那么会将`FiberNode(div-A)`节点传入`completeUnitOfWork`函数，进入`complete`阶段。
...
上面代码之前完成之后，会进入`completeUnitOfWork`函数中的下面代码，获取当前节点的兄弟元素，如果存在兄弟元素，赋值给`workInProgress`变量，重新进入`beginWork`阶段，执行后其`workInProgress.child === null`，则继续执行`completeUnitOfWork`函数：
...
执行完成`completeWork`的`FiberNode(div-B)`节点后，发现其没有兄弟节点，则进入如下代码，获取其当前节点的父节点，并继续执行`do...while`循环：
...
这个过程需要注意📢的是，在`appendAllChildren`函数中，会将其`child`以及其所有兄弟元素插入到当前`FiberNode(div)`的`DOM`中，也就是，会将`FiberNode(div-A)`以及`FiberNode(div-B)`的`DOM`节点都插入进去，也就是上面所说的，会构建以当前节点为**根节点**的完整`DOM`树。除此之外，会执行`bubbleProperties`函数，将其子节点的`flags`属性合并到当前节点的`subtreeFlags`属性中。

内存中数据模型如下图：
![alt text](./image-1.png)

完成当前节点`FiberNode(div)`的`complete`节点，发现其没有`sibling`节点，则继续获取其父节点`returnFiber`继续执行`completeWork`函数，在该函数内根据类型判断为`FunctionComponent`，则进入`bubbleProperties`函数。这是与上面的最大区别，上面执行时起`FiberNode`的类型为`HostComponent`，所以执行创建以及插入`DOM`操作，而`FunctionComponent`类型是在`react`中定义的组件类型，没有对应的宿主元素，只需要执行`bubbleProperties`函数即可，也就是将当前节点的子节点的属性冒泡到当前节点中，方便在后续`commit`阶段对父节点的属性进行对比，直接挂载整个`DOM`树，减少对`FiberTree`的遍历操作。
...
完成当前节点`FiberNode(A)`的`complete`节点，发现其没有`sibling`节点，则继续获取其父节点`returnFiber`，其父节点为`FiberNode(App)`，继续执行`completeWork`函数，在该函数内根据类型判断为`FunctionComponent`，则进入`bubbleProperties`函数。流程图如下：

![alt text](./image-4.png)
与上面流程不同的是，当前的`FiberNode(App)`在`beginWork`阶段创建`FiberNode`时，执行了下面代码，将其`flags`设置为了`Placement`
...
完成当前节点`FiberNode(App)`的`complete`节点，发现其没有`sibling`节点，则继续获取其父节点`returnFiber`继续执行`completeWork`函数，其父节点为`FiberNode(HostRoot)`，在该函数内根据类型判断为`HostRoot`，继续执行`bubbleProperties`函数。
...
完成当前节点`FiberNode(HostRoot)`的`complete`节点，发现其没有`sibling`节点，将其`workInProgress`设置为父节点`returnFiber`，当前节点的父节点`return`为`null`，则跳出整个`workLoopSync`循环，完成了整个`render`阶段。整个`complete`阶段的流程图如下：
