...
在`reconcileChildren`函数中主要执行了下面的过程：
...
在该函数中主要判断了当前`current`，也就是当前执行的`fiberNode`节点是否存在，如果存在，则执行更新`update`操作，如果不存在，则执行`mount`操作

由于当前节点为`RootFiberNode`，是一个已经存在的节点，则执行上面的更新逻辑，也就是`reconcileChildFibers`函数
...
当前情况下传入的`returnFiber`的值为`workInProgress`，`currentFirstChild`的值为`null`, `newChild`的值为`ReactElement<App />`

上面的内容根据情况会走入`reconcileSingleElement`函数中，该函数的主要功能是根据`ReactElement`节点创建对应的`FiberNode`对象，并将其使用`return`指针指向父节点，也就是当前的`workInProgress`节点
...
执行完成上面两个函数之后，内存模型为下图：
![alt text](image-9.png)

执行完成后会把新创建的`FiberNode`返回，然后将其父节点的`child`指针指向该实例，此时内存模型如下：
...
在`updateHostRoot`执行完成后，会返回创建的`FiberNode(App)`的实例，继续执行`beginWork`函数也会将该值返回，则继续执行调用栈上面的函数`performUnitOfWork`

```ts
function performUnitOfWork(fiber: FiberNode) {
  // 执行完成，此时next -> FiberNode(App)
  const next = beginWork(fiber, workInProgressRootRenderLane);
  // 执行完beginWork后，pendingProps 变为 memoizedProps
  fiber.memoizedProps = fiber.pendingProps;
  if (next === null) {
    ...
    // 最后走向这里，将workInProgress指向FiberNode(App)
    // 子节点存在，则继续workLoop
    workInProgress = next;
  }
}
...
根据下面函数执行栈可以看出，当前函数执行完成后，会弹出执行栈，执行后面得`workLoop`函数，在该函数中，判断当前`workInProgress`有值，则会继续执行`performUnitOfWork`函数。
